...
// TODO: change to 0 for performance
    ...
    def impl(self):
        ...
        # Queue
        RxEnq, RxDeq, RxScan = queue_smart2.smart_queue("rx_queue", 64000, n_cores, 2, enq_output=True, enq_blocking=True) # TODO: enq_blocking = False
        TxEnq, TxDeq, TxScan = queue_smart2.smart_queue("tx_queue", 64000, n_cores, 4, clean="enq", enq_blocking=True, enq_atomic=True)
        FullEnq, FullDeq, FullScan = queue_smart2.smart_queue("full_rx_queue", n_cores * 16 * 8, 1, 1, enq_blocking=True)
        rx_enq = RxEnq()
        rx_deq = RxDeq()
        tx_enq = TxEnq()
        tx_deq = TxDeq()
        ...
        class full_segment(API):
            ...
            def impl(self):
                ...
                self.inp >> full_deq
                full_deq.out[0] >> new_segment >> tx_enq.inp[3]
        ...
        class init_segment(API):
            ...
            def impl(self):
                self.inp >> main.FirstSegment() >> tx_enq.inp[3]
        ...
        full_segment('full_segment', process='app')
