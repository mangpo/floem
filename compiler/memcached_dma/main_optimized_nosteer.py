...
output switch { case segment: out(); else: null(); }
    ...
    def impl(self):
        ...
        # Queue
        RxEnq, RxDeq, RxScan = queue_smart2.smart_queue("rx_queue", 512 * 64, nic_rx_threads, 2, overlap=64, #64
                                                        enq_output=True, enq_blocking=True, enq_atomic=False)  # enq_blocking=False?
        rx_enq = RxEnq()
        rx_deq = RxDeq()

        TxEnq, TxDeq, TxScan = queue_smart2.smart_queue("tx_queue", 512 * 160, nic_rx_threads, 1, overlap=160, #160
                                                        checksum=True,
