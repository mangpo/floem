...
output { out(); }
    ...
    bool normal = true;
    if(core_id == 0 && round == 1000) {
        normal = false;
        round = 0;
    }

    output switch {
      case(normal): out(core);
      else: log(0);
      }''' % nic_rx_threads)
...
output switch { case segment: out(); else: null(); }
    ...
    def impl(self):
        ...
        # Queue
        RxEnq, RxDeq, RxScan = queue_smart2.smart_queue("rx_queue", entry_size=64, size=512,
                                                        insts=nic_rx_threads, channels=2, enq_blocking=True,
                                                        enq_atomic=False, enq_output=True)  # enq_blocking=False?
        rx_enq = RxEnq()
        rx_deq = RxDeq()

        TxEnq, TxDeq, TxScan = queue_smart2.smart_queue("tx_queue", entry_size=160, size=512,
                                                        insts=nic_rx_threads, channels=1, checksum=True,
                                                        enq_blocking=True, deq_atomic=False, enq_output=True)
        tx_enq = TxEnq()
        tx_deq = TxDeq()
        ...
        class process_eq(API):
            ...
            def impl(self):
                ...
                self.inp >> main.CpuScheduler() >> rx_deq
        ...
        nic_rx('nic_rx', device=target.CAVIUM, cores=[i for i in range(nic_rx_threads)])
        process_eq('process_eq', process='app')
        init_segment('init_segment', process='app')
        create_segment('create_segment', process='app')
        nic_tx('nic_tx', device=target.CAVIUM, cores=[nic_rx_threads + i for i in range(nic_tx_threads)])
