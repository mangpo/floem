...
// common-case receive
KnownConnection@NIC (in_order) >> (in) InOrder@NIC (to_app) >> (enqueueRx) MainQueue (dequeueRx) 
  <> recvInorder@API;
...
FROM_NET >> CheckConnection@NIC (unknown) >> UnknwonConnection@NIC >> (nic_put) ExceptionQueue (kernel_get)
...
(hs_syn_ack) ConnectionManager@KERNEL >> (kernel_put) ContextQueue (app_get)
  <> NewConnection@APP_CORE (check_clien_request) <> new_connection_ready@API;
